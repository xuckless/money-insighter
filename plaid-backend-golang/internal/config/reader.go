...
// example fails at startup instead of at the first Plaid call.
...
// positiveUint32 parses key as an unsigned 32-bit integer greater than zero.
func (r *reader) positiveUint32(key string, def uint32) uint32 {
	v, present := r.value(key)
	if !present {
		return def
	}
	...
	if err != nil || n == 0 {
		r.fail(key, "must be a positive integer")
		return def
	}
	return uint32(n)
}
