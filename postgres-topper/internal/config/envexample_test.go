...
// The database URL .env.example must carry, matching the local plaidsync Compose database (see Makefile db-init-topper).
const composeDatabaseURL = "postgres://topper:topper@127.0.0.1:5433/plaidsync?sslmode=disable"
...
func TestEnvExampleValuesAreTheDefaults(t *testing.T) {
	...
	example[envAPIKeys] = testAPIKeys
...
func TestEnvExampleHasNoRealSecrets(t *testing.T) {
	for _, e := range parseEnvExample(t) {
		...
		case envWriteTables, envCORSOrigins:
			if e.value != "" {
