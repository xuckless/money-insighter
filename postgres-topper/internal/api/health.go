// Package api holds the HTTP handlers of topper: the two unauthenticated
// probes (/healthz, /readyz), the authenticated generic table endpoints and
// views under /v1, and the middleware that wraps them.
//
// Every response from this package is JSON, and no response ever carries an
// API token, a database URL, or any other secret. The readiness probe in
